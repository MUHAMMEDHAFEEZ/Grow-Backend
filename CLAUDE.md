...
- 026-teacher-school-verification: Strengthened school isolation in teacher signup & login. `validate_and_consume_code` now does 2-phase validation (exists check → school match) with distinct error messages. `login_teacher` gives specific "You are not registered in this school." on school mismatch instead of generic error. (`schools/services/registration_code_service.py`, `teachers/services.py`)
