...
- 025-ai-chat-gemini: Switched AI chat from broken OpenAI (missing `openai` package, wrong FK filters) to Google Gemini (`google-generativeai`, already in requirements). Fixed `ai/selectors.py` — all filters use `submission__student` for Grade queries (Grade has no `student` field), `student_courses` for assignments, and pass `request.user` instead of `student_profile`. Added `AI_API_KEY` env var to `settings.py` and `.env.example`. (`ai/services.py`, `ai/views.py`, `ai/selectors.py`, `grow/settings.py`, `.env.example`)
- 026-teacher-school-verification: Strengthened school isolation in teacher signup & login. `validate_and_consume_code` now does 2-phase validation (exists check → school match) with distinct error messages. `login_teacher` gives specific "You are not registered in this school." on school mismatch instead of generic error. Blocked teacher role from generic `/api/v1/auth/register/` endpoint and blocked teacher login from generic `/api/v1/auth/login/` — teachers must use dedicated teacher endpoints with school verification. (`schools/services/registration_code_service.py`, `teachers/services.py`, `accounts/serializers.py`, `accounts/services.py`)
...
- 023-parent-link-student: Parent-student linking via 4-field verification (school_id, full_name, student_id, grade_id) — looks up Student directly by student_id, validates school/grade/name match; no RegistrationCode involvement; rate limiting via StudentAddRateLimit (5 attempts/hour); duplicate /students/add-student/ endpoint removed (023-parent-link-student)
