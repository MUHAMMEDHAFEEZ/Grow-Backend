...
class CourseViewSet(ViewSet):
    ...
    def get_permissions(self):
        if self.action == "lessons" and self.request.method in ("POST", "PUT", "PATCH", "DELETE"):
            return [IsTeacher()]
    ...
    @extend_schema(
        ...
        responses={
            201: OpenApiResponse(response=CourseSerializer, description="Course created."),
        ...
        examples=[
            OpenApiExample(
                "Create Course",
                value={"title": "Biology — Grade 10", "description": "Topics: cells, genetics, ecosystems."},
                request_only=True,
            ),
        ],
    )
    def create(self, request: Request) -> Response:
        ...
        course = services.create_course(teacher=request.user, **serializer.validated_data)
    ...
    def update(self, request: Request, pk: int = None) -> Response:
        ...
        serializer.is_valid(raise_exception=True)
        course = services.update_course(course_id=pk, teacher=request.user, **serializer.validated_data)
    ...
    @extend_schema(
        ...
        request=None,
        responses={
            201: OpenApiResponse(response=EnrollmentSerializer, description="Enrolled successfully."),
    ...
    @extend_schema(
        ...
        responses={
            201: OpenApiResponse(response=LessonSerializer, description="Lesson created."),
            400: OpenApiResponse(description="Validation error."),
            403: OpenApiResponse(description="You do not own this course."),
        },
        examples=[
    ...
    def lessons(self, request: Request, pk: int = None) -> Response:
        ...
        serializer.is_valid(raise_exception=True)
        lesson = services.create_lesson(course_id=pk, teacher=request.user, **serializer.validated_data)
