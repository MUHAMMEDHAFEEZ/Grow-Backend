...
def get_child_for_parent(parent_user: User) -> User | None:
    ...
    profile = (
        ParentProfile.objects.select_related("child").filter(parent=parent_user).first()
    )
...
def get_parent_ids_for_student(student_id: int) -> list[int]:
    ...
    return list(
        ParentProfile.objects.filter(child_id=student_id).values_list(
            "parent_id", flat=True
        )
    )
...
def get_enrollment_codes_for_school(
    *, school_id: int, admin: User
    ...
    return (
        EnrollmentCode.objects.filter(school=school)
